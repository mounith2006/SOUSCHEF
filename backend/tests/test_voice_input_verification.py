...
class TestVoiceInputVerification(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_I_early_end_of_speech_simulation(self):
        """Verify silence detection logic breaks recording loop early before max duration."""
        from app.services.stt_service import STTService
        import numpy as np

        stt = STTService()
        # Mock VAD energy calculation
        energies = [0.05, 0.05, 0.05, 0.0001, 0.0001, 0.0001]
        
        class DummyStream:
            def __init__(self):
                self.count = 0
            def read(self, chunk_size):
                energy = energies[min(self.count, len(energies) - 1)]
                self.count += 1
                # Return dummy audio array with matching amplitude
                arr = np.full((chunk_size, 1), energy, dtype=np.float32)
                return arr, False
